...
export function TodayTasks({ tasks, overdueTasks }: TodayTasksProps) {
  ...
  return (
    <div className="card overflow-hidden" aria-label="Today's tasks">
      <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <Link href="/tasks" className="flex items-center gap-1 text-xs text-primary-400 hover:text-primary-300 transition-colors font-medium">
          View all <ArrowRight aria-hidden="true" className="w-3 h-3" />
        </Link>
      </div>
          ...
          {allTasks.map((task, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {completing === task.id
                    ? <CheckCircle2 aria-hidden="true" className="w-4 h-4 text-primary-400" />
                    : <Circle aria-hidden="true" className="w-4 h-4 text-text-muted group-hover:text-primary-400 transition-colors" />
                  }
                </button>
